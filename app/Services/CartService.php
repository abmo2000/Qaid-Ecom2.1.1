...
class CartService
{
    ...
    public function addProduct(string $type, int $productId, int $quantity = 1, array $attributes = []): CartItem
    {
        ...
        $this->assertStockAvailability($product, $quantity);

        return $this->updateOrCreateCartItem($product, $quantity, $attributes);
    }
    ...
    public function getItems(): Collection
    {
        return CartItem::query()
            ->with('product')
            ->when(!Auth::check(), fn ($q) => $q->where('session_id', $this->getSessionId()), fn ($q) => $q->where('user_id', Auth::id()))
            ->get()
            ->map(function ($item) {
                ...
                $price = ($item->product_type === 'product' && $product->hasSale())
                    ? $product?->sale?->sale_price
                    : $product->getCartPrice();
                ...
                $attributes = [
                    ...
                    'out_of_stock' => ! $product || $product->getAttribute('in_stock') === false || ($product->getAttribute('stock') !== null && $product->getAttribute('stock') <= 0),
                ];
    ...
    protected function updateOrCreateCartItem(Cartable $product, int $quantity, array $attributes = []): CartItem
    {
        ...
        $this->assertStockAvailability($product, $cartItem->quantity + $quantity);
    ...
    protected function assertStockAvailability(Cartable $product, int $quantity): void
    {
        ...
        if ($stock !== null && $quantity > $stock) {
            throw new \Exception("Only {$stock} item(s) available for {$product->getCartName()}.");
        }
    }

    protected function isProductOutOfStock(Cartable $product): bool
    {
        if (method_exists($product, 'getAttribute') && $product->getAttribute('in_stock') === false) {
            return true;
        }
    ...
    protected function getProductStock(Cartable $product): ?int
    {
